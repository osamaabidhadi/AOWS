...
class ImgStg():
    ...
    def save_img(self,path,name=None):
        if name==None:
            name = fileHandler.get_name_dir("embeded_img_oaw","png", path=path)

        self.embed_img.save(compatible_path(path + "\\" + name))
    
    def save_txt(self,path,name=None):
        if name==None:
            name = fileHandler.get_name_dir("extracted_text_oaw","txt",path=path)
    ...
    def _merge_txt(self, img_p, txt_p, dest_p, showInfo=True):
        ...
        if showInfo:
            messagebox.showinfo("Succesful !", "Al-Wrafy Says: Data embedding was successful.")
    ...
    def _unmerge_txt(self, img_p, dest_p, showInfo=True):
        ...
        if showInfo:
            messagebox.showinfo("Succesful !", "Al-Wrafy Says: Data extraction was successful.")
